...
function ManualEvents({ date }) {
  ...
  const addEvent = () => {
    ...
    const ev = { id: Date.now(), date, time: draft.time || "00:00", title: draft.title, place: draft.place };
...
function TabAgenda() {
  ...
  const addEvent = () => {
    if (!draft.title) return;
    const ev = { id: Date.now(), date: selected, time: draft.time || "00:00", title: draft.title, place: draft.place };
    const next = [...allEvents, ev].sort((a, b) => (a.date + a.time).localeCompare(b.date + b.time));
    setAllEvents(next);
    saveEvents(next);
    setDraft({ time: "", title: "", place: "" });

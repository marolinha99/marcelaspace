...
async function fetchNews(category = "Geral") {
  try {
    ...
    const res = await fetch(
      `https://gnews.io/api/v4/search?q=${q}&lang=pt&country=br&max=10&apikey=${NEWSAPI_KEY}`
    );
    const d = await res.json();
    if (!d.articles) return fallbackNews(category);
